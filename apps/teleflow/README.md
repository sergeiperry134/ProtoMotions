...
`POST /api/accounts/{id}/check` проверяет собственный подключённый аккаунт: принимает ли Telegram сессию, совпадает ли владелец и выставлен ли у профиля флаг ограничений (`restricted` с причиной, если Telegram её сообщает). Результат сохраняется как `health` (`ok`, `restricted`, `unauthorized`) со временем проверки. Если Telegram перестал принимать сессию (например, её завершили в «Устройствах»), любое действие помечает аккаунт как `unauthorized`, а попытка ответа отклоняется до отправки и не блокирует диалог. Это не рейтинг «живучести» и не прогноз блокировки. `PATCH /api/accounts/{id}` с `{"label": "..."}` меняет только метку в TeleFlow, профиль Telegram не трогает.

Управление профилем подключённого аккаунта: `GET/PUT /api/accounts/{id}/profile` — имя (1–64), фамилия (≤64), username (5–32 латинских символов, начинается с буквы; пустое значение убирает username) и био (≤140). Изменения применяются к аккаунту в Telegram и видны всем; смена username меняет ваш @адрес, по которому вас находят, а занятый или недопустимый username Telegram отклонит — панель покажет его сообщение. Это ручная настройка **одного своего** аккаунта: пакетного изменения профилей, ротации «оформления» между аккаунтами и имитации «разных людей» в TeleFlow нет и не будет — такие схемы используются для маскировки массных операций.
...
Schedules accept UTC ISO timestamps (for example `2026-11-03T15:30:00Z`). `POST /api/campaigns/{id}/cancel` returns a scheduled campaign to draft before activation, so a mistaken plan can be stopped ahead of time; a campaign already dispatching cannot be cancelled this way. `GET /api/campaigns/{id}/deliveries?status=failed&limit=50&offset=0` returns per-recipient delivery status, attempts and the last Telegram error, failures first (`status` is optional; `limit` is 1–200). Dispatch is rate-limited and persisted in SQLite. A restart resumes queued work; if the process stops after Telegram accepted a send but before SQLite recorded it, that single delivery can be retried, so delivery is at-least-once around a crash. Transient campaign and keyword-reply failures (including Telegram `retry_after`/429, 5xx and network timeouts) are persisted with bounded backoff and retried up to eight attempts; permanent errors count as failures. When a campaign has failed deliveries, an operator can explicitly retry only those recipients from the campaign list; successful deliveries are never requeued, and unsubscribed contacts are skipped. A scheduled campaign with no recipients returns to draft. An operator can also opt a subscriber out through `PATCH /api/subscribers/{id}` with `{"opted_in": false}`; pending keyword replies for that chat are cancelled immediately, and consent can only be restored by the subscriber sending `/start` again. Manual inbox replies return transient API failures to the caller with `Retry-After` when Telegram provides it, so the client can retry explicitly. Only one Teleflow process may own a database at a time; a sidecar advisory lock prevents a second worker from recovering active sends. AI drafts, replies, campaigns, and stored inbox history should be treated as sensitive; protect, back up, and delete the SQLite file according to your retention policy.

**Welcome funnel.** `GET /api/sequence` returns the single welcome sequence and its run counts; `PUT /api/sequence` with `{"enabled": true, "steps": [{"delay_minutes": 0, "body": "..."}]}` replaces it (at most 10 steps, delays 0–43 200 minutes, each measured from the previous message). A subscriber is enrolled only when they send `/start` while the sequence is enabled; `/stop` or an operator opt-out stops their run, and sending `/start` again restarts only a stopped or failed run, never an active or completed one. Consent is re-checked right before every step. Disabling the sequence pauses delivery; editing steps applies to active runs from their next step, and shortening the list completes runs beyond the new end. Steps share the campaign retry policy, are logged in the inbox history and, like campaigns, are at-least-once around a crash.

**Placeholders.** Subscriber campaigns and funnel steps may use `{first_name}` and `{username}` (without `@`); missing values render as empty text. They are rejected for channel/group campaigns and when the worst-case expansion could exceed 4096 characters.

**Subscriber data.** `GET /api/subscribers/export` downloads the bot's own subscriber list as UTF-8 CSV (with a BOM for spreadsheets); cells that start with `=`, `+`, `-` or `@` are prefixed with `'` so spreadsheet apps do not evaluate them. `DELETE /api/subscribers/{id}` erases a subscriber's profile, message history, queued replies and funnel run; campaign delivery rows are kept for totals but lose the identifier (queued deliveries to that person are skipped).

**Retention.** The worker prunes processed-update markers older than 7 days (unless a keyword reply for them is still pending) and settled account-reply records older than 30 days; unreviewed unknown outcomes are kept until an operator reviews them. Message history and campaign reports are not pruned automatically.
